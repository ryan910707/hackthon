...
from package.winker import wink_detection

from package.inpaint import Sketcher
...
class SetupWindow(QWidget):
    ...
    def init_ui(self):
        # self.selected_folder="C:\Users\Kenneth Yang\Desktop\hackathon2022\hackthon\data"
        # self.continue_app()
        # self.selectFolderDialog = QFileDialog.getExistingDirectory(self, 'Select directory')
        self.setWindowTitle('ASML--227')
        ...
        try:
            styles_path = os.path.join(
                r"C:\Users\Ryanke\Desktop\hackthon\image_labeler\styles.qss")
    ...
    def continue_app(self):
        ...
        if form_is_valid:
            ...
            LabelerWindow(label_values, self.selected_folder,
                          self.mode).showMaximized()
        else:
...
class LabelerWindow(QWidget):
    def __init__(self, labels, input_folder, mode):
        ...
        self.blink_success_label = QLabel(self)

        self.tony_success_label = QLabel(self)
        self.waylon_success_label = QLabel(self)

        self.blink_button = QtWidgets.QPushButton("Blink", self)

        # state variables
        self.counter = 0
        self.input_folder = input_folder
        self.selected_folder = input_folder
        self.img_paths = get_img_paths(input_folder)
        self.labels = ["tony", "waylon"]
        ...
        self.output_image_box = QLabel(self)
        ...
        self.generate_xlsx_checkbox = QCheckBox(
            "Also generate .xlsx file", self)

        # init UI
        self.init_ui()

    def check_validity(self, folder_path):
        """
        :return: if all the necessary information is provided for proper run of application. And error message
        """
        if folder_path == '':
    ...
    def pick_new(self):
        ...
        form_is_valid, message = self.check_validity(folder_path)
        ...
        self.img_paths = get_img_paths(folder_path)

        # show image
        self.set_image(self.img_paths[0])
        self.set_output_image(self.img_paths[0])
        self.image_box.setGeometry(20, 120, self.img_panel_width, self.img_panel_height)
        self.image_box.setAlignment(Qt.AlignTop)


        self.output_image_box.setGeometry(20+self.img_panel_width+125, 120, self.img_panel_width, self.img_panel_height)
        self.output_image_box.setAlignment(Qt.AlignTop)
        ...
        self.blink_success_label.hide()

        # create label folders
        # if self.mode == 'copy' or self.mode == 'move':
        #     self.create_label_folders(self.labels, folder_path)

    def blink_separate(self):
        ...
        if not form_is_valid:
            print(message)
            return
        if "not_blink" in self.img_paths[0]:
            self.blink_success_label.setText("Already processed !")
            return
        for i in range(len(self.img_paths)):
            ...
            copy_to = os.path.join(self.input_folder, label, img_name)
            print("copy to:  ", copy_to)
            if self.mode == 'copy':
                if(not wink_detection(img_path)):
                    shutil.copy(img_path, copy_to)
                    self.blink_success_label.show()
            elif self.mode == 'move':
                if(not wink_detection(img_path)):
    ...
    def init_ui(self):
        ...
        # image headline
        self.curr_image_headline.setGeometry(20, 30, 300, 20)
        self.curr_image_headline.setObjectName('headline')
        self.selected_folder_label.setGeometry(400, 0, 550, 26)
        self.selected_folder_label.setObjectName("selectedFolderLabel")

        # selcect folder
        self.headline_folder = QLabel(
            'Select folder containing images you want to label', self)
        self.headline_folder.setGeometry(20, 0, 500, 20)
        self.headline_folder.setObjectName("headline")

        self.browse_button = QtWidgets.QPushButton("Browse", self)
        self.browse_button.setGeometry(1000, 0, 80, 28)
        self.browse_button.clicked.connect(self.pick_new)

        self.blink_button.setGeometry(1100, 0, 80, 28)
        self.blink_button.clicked.connect(self.blink_separate)

        self.blink_success_label = QLabel(self)
        self.blink_success_label.setGeometry(1185, 5, 100, 20)
        self.blink_success_label.setObjectName("blink_success")
        self.blink_success_label.setStyleSheet('color: #43A047')
        self.blink_success_label.setText("Success !")
        self.blink_success_label.hide()

        self.tony_success_label = QLabel(self)
        self.tony_success_label.setGeometry(770, 125, 100, 20)
        self.tony_success_label.setObjectName("blink_success")
        self.tony_success_label.setStyleSheet('color: #43A047')
        self.tony_success_label.setText("Success !")
        self.tony_success_label.hide()

        self.waylon_success_label = QLabel(self)
        self.waylon_success_label.setGeometry(770, 170, 100, 20)
        self.waylon_success_label.setObjectName("blink_success")
        self.waylon_success_label.setStyleSheet('color: #43A047')
        self.waylon_success_label.setText("Success !")
        self.waylon_success_label.hide()

        # image name label
        self.img_name_label.setGeometry(20, 50, self.img_panel_width, 20)

        # progress bar (how many images have I labeled so far)
        self.progress_bar.setGeometry(20, 75, self.img_panel_width, 20)

        # csv note
        self.csv_note.setGeometry(self.img_panel_width + 20, 640, 400, 20)

        # message that csv was generated
        self.csv_generated_message.setGeometry(
            self.img_panel_width + 20, 660, 800, 20)
    ...
    def waylon(self, img_path):
        return Sketcher.remove(img_path)

    def tony(self, img_path):
        print("tony")
        # return combined_display(img_path)
        return os.path.join(r'C:\Users\Ryanke\Desktop\hackthon\data\test\tony.jpg')
    ...
    def set_label(self, name):
        ...
        if name == "tony":
            ...
            directory = os.path.join(self.selected_folder, "output")
            ...
            if img_name in self.assigned_labels.keys():
                copy_to = directory + "\\"+img_name
                print("the image now labeling: ", img_path)
                # means tht user want's to add label
                if label not in self.assigned_labels[img_name]:

                    self.assigned_labels[img_name].append(label)

                    # WRITE your FUCKING function here, use the image path as input
                    copy_from = self.tony(img_path)
                    # path to copy/move images
                    # copy_to = os.path.join(self.input_folder, "output")

                    # copy/move the image into appropriate label folder
                    if self.mode == 'copy':
                        # the image is stored in input_folder, so i can copy it from there (differs from 'move' option)
                        copy_from = os.path.join(
                            self.input_folder, "output", img_name)
                        ...
                        copy_from = os.path.join(
                            self.input_folder, "output", img_name)
                        shutil.move(copy_from, copy_to)
                    self.tony_success_label.show()
                    self.set_output_image(copy_to)
            ...
            if img_name in self.assigned_labels.keys():
                ...
                if label not in self.assigned_labels[img_name]:
                    ...
                    # WRITE your FUCKING function here, use the image path as input
                    copy_from = self.waylon(img_path)
                    # path to copy/move images
                    # copy_to = os.path.join(self.input_folder, "output")
                    ...
                    if self.mode == 'copy':
                        ...
                        shutil.move(copy_from, copy_to)
                        # os.remove(img_path)
                    self.waylon_success_label.show()
                    self.set_output_image(copy_to)
    ...
    def set_output_image(self, path):
        ...
        self.output_image_box.setPixmap(pixmap)
